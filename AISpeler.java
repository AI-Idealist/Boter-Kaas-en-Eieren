...
public class AISpeler {
   ...
   public int[] KiestZetIntelligent() {
    ...
    int[] result = minimax(4, "agent");
     zet[0] = result[1];
     zet[1] = result[2];
    return zet; 
    }
   
   //  Recursive minimax at level of diepte for either maximizing or minimizing speler.
   //  Return int[3] of {score, row, col}  
   private int[] minimax(int diepte, String speler) {
   
      // Bepaal mogelijke zetten
       List<int[]> mzetten = BepaalMogelijkeZetten();
      
      // agent is maximizing; while gebruiker is minimizing
      int hiscore = (speler.equals("gebruiker")) ? Integer.MAX_VALUE : Integer.MIN_VALUE;
      int score;
      int bestRow = -1;
      int bestCol = -1;
      
      if ( mzetten.isEmpty() || diepte == 0) {
         // Gameover or diepte reached, evaluate score
         hiscore = EvalueerOpstelling();
      } else {
         for (int[] mzet : mzetten) {
            ...
            if (speler.equals("gebruiker")) {lcellen[kolom][rij] = bord.x;}
            else {lcellen[kolom][rij] = bord.o;}
            
            this.printbord();
            
            if (speler.equals("agent")) {  // mySeed (computer) is maximizing speler
              
               score = minimax(diepte - 1, "gebruiker")[0];
                //System.out.println("************************");
                //System.out.println("Cniveau :" + diepte + " " + "k: " + kolom + " r: " + rij + " score : " + score + " hiscore : " + hiscore + " beste Kol : " + bestCol + "beste Rij " + bestRow);
                //this.printbord();
                // System.out.println("************************");
               if (score > hiscore) {
                  hiscore = score;
               ...
               score = minimax(diepte - 1, "agent")[0];
                // System.out.println("************************");
                // System.out.println("Aniveau :" + diepte + " " + "k: " + kolom + " r: " + rij + " score : " + score + " hiscore : " + hiscore + " beste Kol : " + bestCol + "beste Rij " + bestRow);
                // this.printbord();
                // System.out.println("************************");
               if (score < hiscore) {
                  hiscore = score;
                  bestCol = mzet[0];
                  bestRow = mzet[1];
                  //System.out.println("niveau :" + diepte + " " + "k: " + kolom + " r: " + rij + " score : " + score + " hiscore : " + hiscore + " beste Kol : " + bestCol + "beste Rij " + bestRow);
        
               }
            }
            // Undo mzet
            lcellen[kolom][rij] = bord.e;
        }
      }
      return new int[] {hiscore, bestCol, bestRow};
   }
